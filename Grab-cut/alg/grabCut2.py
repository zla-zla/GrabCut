...
# @jit(nopython=True)
def construct_gc_graph(img,mask,fgd_gmm,bgd_gmm,rows,cols,left_V,upleft_V,up_V,upright_V):
...
def GrabCut(img, mask, rect):
    ...
    for ii in range(q):
        ...
        tt = time.time()-t5
        t_gmm += tt
        print(f'初始化两个GMM用时：{round(tt,5)}s')

        # 创建图
        T3 = time.time()
        gc_graph, nodes = construct_gc_graph(img,mask,fgd_gmm,bgd_gmm,rows,cols,left_V,upleft_V,up_V,upright_V)
        tt=time.time()-T3
        t_build+=tt
        print(f'创建图用时：{round(tt,5)}s')
        ...
        tt=time.time() - T1
        t_cut+=tt
        print(f'计算最小割用时：{round(tt, 5)}s')
        # if(last-value>0.01*last):
        #     last = value
        # else:
        #     break
    print(f'总计创建图用时：{t_build},gmm初始化用时：{t_gmm},最小割用时：{t_cut}')
    return mask
