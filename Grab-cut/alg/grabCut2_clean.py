...
def GrabCut(img, mask, rect, component=5):
    ...
    # for i in range(4):
        ...
        t5=time.time()
        bgd_model = Model(img[bgd_indexes], component=component)  # 输入背景像素的序号，创建背景model
        fgd_model = Model(img[fgd_indexes], component=component)  # 输入前景像素的序号，创建前景model
        tt = time.time() - t5
        t_model += tt
