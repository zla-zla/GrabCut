...
def construct_base_graph(cols, rows, mask, left_V, upleft_V, up_V, upright_V):
...
def estimate_segmentation(mask,gc_graph,rows,cols,nodes):
    ...
    mask[pr_indexes] = np.where(np.isin(img_indexes[pr_indexes], bg_indexs), BG,PR_FG)
...
def GrabCut(img, mask, rect):
    ...
    ################################################ 创建基本图 ################################################
    base_graph, nodes = construct_base_graph(cols, rows, mask, left_V, upleft_V, up_V, upright_V)
    ...
    while(True):
        ...
        # bgd_indexes（背景和未确定背景）；fgd_indexes（前景和未确定前景）
        bgd_indexes, fgd_indexes = bg_mask(mask), fg_mask(mask)
        print('可能的背景数量为: %d, 可能的前景数量为: %d' % (bgd_indexes[0].size, fgd_indexes[0].size))

        bgd_model = Model(img[bgd_indexes])  # 输入背景像素的序号，创建背景model
        fgd_model = Model(img[fgd_indexes])  # 输入前景像素的序号，创建前景model
        ...
        if(last-value<0.01*last or i >10):
            ...
            last = value
    print(f'总计创建图用时：{t_build},model初始化用时：{t_model},最小割用时：{t_cut}')
    return mask
